...
class PlatooningBehaviorAgent(BehaviorAgent):
    ...
    def joining_finish_manager(self, insert_vehicle='front'):
        ...
        Args:
            -insert_vehicle (string): indicate use the front or rear vehicle
             index to update the platoon manager list.
        """
    ...
    def calculate_gap(self, distance):
        ...
        Args:
            ...
            the ego vehicle and frontal vehicle
        """
    ...
    def platooning_following_manager(self, inter_gap):
        ...
        if len(self._local_planner.get_trajetory()
            ...
            for i in range(tracked_length):
                ...
                if i == 0:
                    pos_x = (
                        frontal_trajectory[i][0].location.x +
                        inter_gap / delta_t * ego_loc_x) / (
                        1 + inter_gap / delta_t)
                    pos_y = (
                        frontal_trajectory[i][0].location.y +
    ...
    def platooning_merge_management(self, frontal_vehicle_vm):
        ...
        Args:
            -frontal_vehicle_vm (opencda object): The vehivlel manager of the
             front vehicle.
        Returns:
            -target_speed (float): The target speed for ego vehicle.
            -target_waypoint (carla.waypoint): The target waaypoint for
             ego vehcile.
        """
        print("start merging !")
        ...
        target_speed, target_waypoint =\
            super().run_step(target_speed=1.5 *
                             get_speed(frontal_vehicle_vm.vehicle),
    ...
    def run_step_cut_in_move2point(self):
        ...
        distance, angle = cal_distance_angle(
            rear_vehicle_vm.vehicle.get_location(),
            ego_vehicle_loc, ego_vehicle_yaw)
    ...
    def run_step_cut_in_joining(self):
        ...
        frontal_lane = self._map.get_waypoint(
            frontal_vehicle.get_location()).lane_id
    ...
    def run_step_back_joining(self):
        ...
        Returns:
            -control command (opencda object): control command.
