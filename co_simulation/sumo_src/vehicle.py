...
class Vehicle():
    ...
    def getClosestDistances(self, veh):
        ...
        all_vehs_around = list(veh[2].keys())
        all_vehs_around.remove(veh_name)
        
        distances = [self.sensorRange]*6
        speeds = [50]*6
        signals = [0]*6
        ...
        elif veh_edge == 'gneE0_1' or veh_edge == 'gneE4_2' or veh_edge == 'gneE5_1' or veh_edge == ':gneJ1_1_1'\
                or veh_edge == ':gneJ6_0_2':
            distances[0:2] = [a if a < self.sensorRange else -1 for a in distances[0:2]]
            speeds[0:2] = [a if a < 50 else -1 for a in speeds[0:2]]
