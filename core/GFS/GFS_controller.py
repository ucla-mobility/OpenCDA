...
class GFSController(object):
    ...
    def getBestMergePosition(self, merge_veh, platooning_manager):
        ...
        # the 4.8 is hard coded, may need to tune
        platoon_vehicles = [manager.vehicle for manager in vehicle_managers if abs(manager.vehicle.get_location().x+4.8) < 1e-3]
    ...
    def getContext(self, vehicle):
        ...
        x,y = vehicle.get_location().x, vehicle.get_location().y
        for v in vehicles:
