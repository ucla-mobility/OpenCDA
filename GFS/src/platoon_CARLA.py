...
'''
Rewarite all traci functions in CARLA 
'''
...
class Platoon():
    ...
    # ! rewrite get lane length in CARLA
    ...
    def getLanePositionFromFront(self, lane=None):
        if lane:
            ...
            if vehiclesInLane:
                ...
                return self.getLaneLength(lane) - vehiclesInLane[0].getLanePositionFromFront()
        else:
            # return traci.lane.getLength(self._lane) - self._lanePosition
            return self.getLaneLength(self._lane) - self._lanePosition
    ...
    def getLength(self):
        ...
        laneLen = self.getLaneLength(self._lane)
    ...
    def startBehaviour(self, vehicles):
        ...
        if self.isActive():
            for v in vehicles:
                ...
                # v.setImperfection(0) -->CARLA does not have this attribute
                v.setMinGap(0)
                # !! traci.vehicle.setTau(v.getName(), 0.6) change to CARLA Vehicle
                v.setTau(0.6)
                # v._state = 'Platooning'
            self.update()
    ...
    def stopBehaviour(self):
        ...
        for v in self._vehicles:
            if v.isActive():
                ...
                # v.setImperfection(0.5) # perfect CAV behavior -->CARLA does not have this attribute
                v.setMinGap(0)
                # traci.vehicle.setTau(v.getName(), 0.6) --> change to CARLA Vehicle
                v.setTau(0.6)
        ...
        for veh in vehicles:
            try:
                if veh.getEdge() == leadVehEdge:
                    # change lane, this should be addressed by CARLA
